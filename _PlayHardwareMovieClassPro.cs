...
public class PlayHardwareMovieClassPro : MonoBehaviour
{
	...
	protected bool movieFinished=false;
	protected bool movieReady = false;
	...
	//called in videoSettings when transition ended)
	public void moviePlaying(){
	
		movieFinished=false;
	}
		
	//called at the end of the video (in videoSettings and in the update of the Main)		
	public bool isMovieFinished(){
		
		return movieFinished;	
	}
	
	//to get the width and height of the video
	public Vector2 movieWH(){
	...
	void MoviePreferredRotation ()
	{
		if (!transform.parent.name.Equals ("MovieProferredRotation")) {
			Debug.LogError ("Movie needs MovieProferredRotation parent to rotate to preferred rotation");	
		} else {
	...
	void OnApplicationPause (bool pause)
	{
		if (pause) {
			...
			ResumeMovie ();// MUST resume after application pause
		}
	}
